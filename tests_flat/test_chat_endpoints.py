...
# Provide a lightweight fake `api.simple_connector` module so importing `api.main` succeeds during tests.
fake_mod = types.ModuleType("api.simple_connector")
...
sys.modules["api.simple_connector"] = fake_mod

# Provide a lightweight fake `api.shared_memory` module used by api.main
fake_shared = types.ModuleType("api.shared_memory")
...
sys.modules["api.shared_memory"] = fake_shared
...
def test_chat_stream_endpoint_monkeypatch(monkeypatch):
    ...
    resp = client.post(
        f"/chat_stream/{role}", json={"message": "Hi", "session_id": "s1"}
    )

...
def load_agent_config(config_path):
    ...
    system_prompt_path = os.path.join(
        Path(config_path).parent, config["prompts"]["system"]
    )
    persona_prompt_path = os.path.join(
        Path(config_path).parent, config["prompts"]["persona"]
    )
...
def run_local_llama_chat_loop(prompt, config):
    ...
    while True:
        ...
        history += f"<|user|>\n{user_input.strip()}</s>\n<|assistant|>\n"
        output = llm(
            history, max_tokens=max_tokens, temperature=temperature, stop=["</s>"]
        )

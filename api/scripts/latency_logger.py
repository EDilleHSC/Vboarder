...
class LatencyLogger:
    def __init__(self, log_file: str = "ingestion_latency.jsonl", target_latency: float = 2.0):
        self.log_file = Path(log_file)
        self.target_latency = target_latency  # seconds
        self.current_session = []
        
    @contextmanager
    def track_ingestion(self, doc_name: str, doc_type: str = "unknown", metadata: Dict = None):
    ...
    def log(self, doc_name: str, doc_type: str, latency: float, 
            timestamp: str, error: Optional[str] = None, metadata: Dict = None):
    ...
    def get_session_stats(self) -> Dict:
        ...
        latencies = [e['latency_seconds'] for e in self.current_session if not e['error']]
        ...
        return {
            ...
            "latency": {
                ...
                "p95": round(statistics.quantiles(latencies, n=20)[18], 3) if len(latencies) > 1 else latencies[0],
            },
            "target_compliance": {
                "target_seconds": self.target_latency,
                "docs_meeting_target": sum(1 for l in latencies if l < self.target_latency),
                "compliance_rate": sum(1 for l in latencies if l < self.target_latency) / len(latencies),
            }
        }
    ...
    def print_session_report(self):
        ...
        if stats.get('latency'):
            ...
            status = "✅" if compliance_pct >= 95 else "⚠️" if compliance_pct >= 80 else "❌"
            
            print(f"\n🎯 Target Compliance ({self.target_latency}s):")
            print(f"  Docs Meeting Target: {target['docs_meeting_target']}/{stats['successful']}")
            print(f"  Compliance Rate: {compliance_pct:.1f}% {status}")
            
            if compliance_pct < 95:
                print(f"\n⚠️ WARNING: Target compliance below 95%")
                print(f"   Consider optimizing pipeline or increasing resources")
        ...
        for doc_type, lats in by_type.items():
            if lats:
                type_stats[doc_type] = {
                    ...
                    "compliance_rate": sum(1 for l in lats if l < self.target_latency) / len(lats)
                }

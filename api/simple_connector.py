...
# Import memory-aware prompt builders for all agents
...
from agents.rag_memory import init_db_pool, search_knowledge_base
from agents.SEC.agent_logic import build_sec_prompt
from api.shared_memory import shared_block_text

# Map agent IDs to prompt builder functions
PROMPT_BUILDERS = {
...
class AgentConnector:
    ...
    def __init__(self, agent_role="ceo", session_id="default"):
        ...
        self.conversation_file = os.path.join(
            self.conversations_dir, f"{self.agent_role}_{self.session_id}.json"
        )
    ...
    def _load_agent_config(self):
        ...
        config = {
            ...
            "role": (
                f"Chief {self.agent_role.upper()[1:]} Officer"
                if self.agent_role != "sec"
                else "Executive Secretary"
            ),
    ...
    async def _build_system_prompt(self, concise: bool):
        style = (
            "Answer in 1-2 short sentences. Do not list steps or principles. Do not restate your role/title. Avoid repetition."
            if concise
            else "Be concise. Max 2-3 short points. Do not restate your role/title. Avoid repetition."
        )

        base_prompt = self.agent_config.get(
            "system_detailed",
            f"You are {self.agent_config['name']}, the {self.agent_config['role']}.",
        )
    ...
    async def chat(self, user_message: str, concise: bool = False):
        """
        Chat with agent using memory-aware prompt building.
        Async method that awaits prompt builder coroutines.
        """
        self.conversation_history = self._load_conversation()
        ...
        # Build memory-enriched system prompt using agent logic
        agent_id = self.agent_role.upper()
        build_prompt = PROMPT_BUILDERS.get(agent_id)

        if build_prompt:
            try:
                system_prompt = await build_prompt(user_message)
            except Exception as e:
                print(f"Warning: Failed to build memory prompt for {agent_id}: {e}")
                system_prompt = await self._build_system_prompt(concise=concise)
        else:
            system_prompt = await self._build_system_prompt(concise=concise)
